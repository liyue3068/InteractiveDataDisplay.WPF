...
namespace InteractiveDataDisplay.WPF
{
    ...
    public class AxisGrid : PlotBase
    {
        ...
        public static readonly DependencyProperty VerticalTicksProperty =
            DependencyProperty.Register("VerticalTicks", typeof(IEnumerable<double>), typeof(AxisGrid), new PropertyMetadata(Array.Empty<double>(),
                (o, e) =>
                {
        ...
        public static readonly DependencyProperty HorizontalTicksProperty =
            DependencyProperty.Register("HorizontalTicks", typeof(IEnumerable<double>), typeof(AxisGrid), new PropertyMetadata(Array.Empty<double>(),
                (o, e) =>
                {
                    AxisGrid axisGrid = (AxisGrid)o;
                    if (axisGrid != null)
        ...
        /// </summary>
        public static new readonly DependencyProperty IsYAxisReversedProperty =
        ...
        public AxisGrid()
        {
            HorizontalTicks = Array.Empty<double>();
            VerticalTicks = Array.Empty<double>();

            path = new Path();
            BindingOperations.SetBinding(path, Path.StrokeProperty, new Binding("Stroke") { Source = this, Mode = BindingMode.OneWay });
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            if (hTicks != null && hTicks.Length > 0)
            {
                ...
                for (; i < hTicks.Length; i++)
                {
                    ...
                    {
                        StartPoint = new Point(screenX, minY),
                        EndPoint = new Point(screenX, maxY)
                    };
            ...
            if (vTicks != null && vTicks.Length > 0)
            {
                ...
                for (; i < vTicks.Length; i++)
                {
                    ...
                    LineGeometry line = new LineGeometry
                    {
                        StartPoint = new Point(minX, screenY),
                        EndPoint = new Point(maxX, screenY)
                    };
                    group.Children.Add(line);
                }
            }
